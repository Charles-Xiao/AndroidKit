...
public class JazzyHelper implements AbsListView.OnScrollListener {
	...
	public final void onScrolled(ViewGroup viewGroup, int firstVisibleItem,
			int visibleItemCount, int totalItemCount) {
		...
		if (mIsScrolling && shouldAnimateItems) {
			...
			while (lastVisibleItem - indexBeforeLast > mLastVisibleItem) {
				View item = viewGroup.getChildAt(lastVisibleItem
		...
		if (mIsScrolling) {
			...
			if (mSimulateGridWithList) {
				...
				for (int i = 0; i < itemRow.getChildCount(); i++)
					doJazzinessImpl(itemRow.getChildAt(i), position,
							scrollDirection);
			} else {
...
//			break;
	...
	private void notifyAdditionalOnScrollListener(AbsListView view,
		...
		if (mAdditionalOnScrollListener != null) {
			mAdditionalOnScrollListener.onScroll(view, firstVisibleItem,
					visibleItemCount, totalItemCount);
		}
	}
